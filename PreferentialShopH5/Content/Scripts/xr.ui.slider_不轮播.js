...
(function ($) {
    ...
    var css = {
        ...
        sliderOps: "sliderOps",
        //      轮播左滚动按钮
        sliderLBtn: "sliderLBtn",
        //      轮播右滚动按钮
        sliderRBtn: "sliderRBtn"
    };
    ...
    function createHtml($this) {
        ...
        //操作图片左滚动按钮
        var leftBtn = $("<a></a>").attr({ "href": "javascript:void(0)" }).addClass(css.sliderLBtn);
        //操作图片右滚动按钮
        var rightBtn = $("<a></a>").attr({ "href": "javascript:void(0)" }).addClass(css.sliderRBtn);
        $.each(options.data, function (i, val) {
                picLists.append($("<li style='width:" + objWidth + "px'></li>").append($("<a></a>").attr({ "href": val.link }).html("<img src='" + val.picUrl + "' class='full-width'/>")));
                sliderLists.append($("<li></li>").append($("<a></a>").attr({ "href": "javascript:void(0)" })));    
        });
        //console.log($(".sliderPics").height());
        picLists.css({ "width": options.data.length * objWidth });
        $this.addClass(css.llbSlider);//.css({ "height": "191px" });
        $this.append(picLists).append(sliderLists).append(leftBtn).append(rightBtn);
    ...
    function _init($this) {
        ...
        var play = function () {
            ...
            if (options.data[_current].isOver == 0 && options.data[_current].status == 1) {
                countTimeDown({ date: options.data[_current].endTime, offset: +8, container: $(".leftTime"), startTime: options.data[_current].startTime }, function () {
                    ...
                    $(".skilledStatue").eq(tabsIndex).html("<img src='/content/images/skilledError.jpg'/>");
                });
            }else{
        ...
        //鼠标移入移出轮播图片上发生的事件响应
        $.each(_sliderPics, function (i, val) {
            $(val).mouseover(function () {
                stop();
            }).mouseout(function () {
                if (options.autoSwitch && _count>1) {
                    auto();
                }
            })
        });
        //鼠标移入移出控制图片可切换的列表上发生的事件响应
        $.each(_sliderLists, function (i, val) {
            $(val).click(function () {
                _current = $(val).index();
                play();
            }).mouseover(function () {
        ...
        //控制图片向前轮播的事件响应
        $("." + css.sliderLBtn).mouseover(function () {
            stop();
        }).click(function () {
            prev();
            stop();
        ...
        //控制图片向后轮播的事件响应
        $("." + css.sliderRBtn).mouseover(function () {
            stop();
        }).click(function () {
            next();
            stop();
